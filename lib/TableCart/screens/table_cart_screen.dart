...
//   }
//
//   // ── GET api/table-requests/cart/{cartId} ────────────────────────────────
...
//   }
//
//   // ── POST api/table-requests/create ─────────────────────────────────────
...
//   }
//
//   // ── PUT api/table-requests/update/{id}?status={status} ─────────────────
...
//   // ── Role State ────────────────────────────────────────────────────────────
//   /// true  → ROLE_VENDOR  → direct decrement / remove without popup
//   /// false → ROLE_EMPLOYEE → show removal request popup on decrement/remove
...
//
//   // ── Table Requests State (GET api/table-requests/cart/{cartId}) ──────────
...
//         });
//
//         // After fetching the cart, also fetch table requests (GET)
//         if (cd.cartId != null) {
...
//   }
//
//   // ─── Existing removal requests (old widget-based model) ─────────────────
...
//
//   // =========================================================================
//   // TABLE REQUESTS — GET api/table-requests/cart/{cartId}
//   // =========================================================================
//
//   /// Fetches all table requests (PENDING / ACCEPT / DECLINE) for the cart.
//   /// Called after every cart refresh so the UI is always up-to-date.
...
//   // Only callable when _isVendorRole == true.
//   // =========================================================================
//
...
//
//     if (success) {
//       // After accepting, update the cart quantity locally
...
//         }
//       } else {
//         _showSnack('Request accepted', success: true);
//       }
//       await _fetchCartFromBackend();
//     } else {
...
//     );
//     if (success) {
//       _showSnack('Request declined', success: false);
//       if (_cartData != null) {
...
//
//   // =========================================================================
//   // ITEM OPERATIONS  — role-based logic
//   // =========================================================================
//
//   // ── API items (already saved on server) ───────────────────────────────────
//
//   Future<void> _increaseApi(CartItem item) async {
//     // Both VENDOR and EMPLOYEE can increase freely
...
//     if (cartId == null) return;
//
//     // ── EMPLOYEE ROLE: always show removal request popup ──────────────────
//     if (!_isVendorRole) {
//       await _showRemovalRequestPopup(item);
//       return;
//     }
//
//     // ── VENDOR ROLE: direct decrement ────────────────────────────────────
//     setState(() => _itemLoadingMap[item.itemId!] = true);
//     try {
//       if (item.quantity <= 1) {
...
//     // Both roles can increase freely for local items
//     setState(() {
...
//     // ── EMPLOYEE ROLE: show removal request popup ─────────────────────────
//     if (!_isVendorRole) {
//       await _showRemovalRequestPopup(item);
//       return;
//     }
...
//         onSuccess: () {
//           _showSnack('Removal request submitted!', success: true);
...
//       await _fetchCartFromBackend();
//       _showSnack('✅ Items saved successfully!', success: true);
//     } catch (e) {
...
//       }
//       await _fetchCartFromBackend();
//       _showSnack('✅ KOT sent! ($updatedCount items)', success: true);
//     } catch (err) {
...
//         }
//       }
//       await _printKOT(bookingId, itemsToUpdate);
//       await _fetchCartFromBackend();
//       _showSnack('✅ $updatedCount items delivered & printed!', success: true);
//     } catch (err) {
...
//       await _printKOT(bookingId, itemsToUpdate);
//       await _fetchCartFromBackend();
//       _showSnack('✅ $updatedCount items KOT printed!', success: true);
//     } catch (err) {
//       _showSnack('Failed: $err');
//     } finally {
//       setState(() => _isKotPrinting = false);
//     }
//   }
//
//   Future<void> _printKOT(int bookingId, List<CartItem> items) async {
//     _showSnack('🖨️ KOT printed for ${items.length} items', success: true);
//   }
//
...
//       });
//       await _fetchCartFromBackend();
//       _showSnack(apply ? '✅ Tip added!' : '✅ Tip removed!', success: true);
//     } catch (e) {
...
//
//   // =========================================================================
//   // VENDOR TABLE REQUESTS BOTTOM SHEET
//   // =========================================================================
//
...
//               children: [
//                 // ── Pending requests banner (vendor only) ──────────────────
...
//                   ],
//                 ),
//                 // Pending request chips (vendor only)
...
//   }
//
//   /// Inline chip shown under a cart item row for VENDOR to accept/decline
...
//                   _isKotPrinting ? null : _handleKOTAndPrint,
//                   loading: _isKotPrinting,
//                 ),
//                 const SizedBox(width: 6),
//                 Expanded(
//                   child: Stack(
...
//           Navigator.pop(ctx);
//           await _fetchCartFromBackend();
//           _showSnack('Table changed successfully!', success: true);
//         },
//       ),
//     );
//   }
...
//                     onPressed: () {
//                       Navigator.pop(ctx);
//                       _showSnack('✅ E-Bill sent!', success: true);
//                     },
//                     child: const Text(
//                       'Send E-Bill',
//                       style: TextStyle(color: Colors.white),
...
//                               Navigator.pop(ctx);
//                               _showSnack(
//                                 '✅ Bill split among $guests guests',
//                                 success: true,
//                               );
//                             }
//                           : null,
...
//
// // =============================================================================
// // PENDING TABLE REQUESTS BOTTOM SHEET  (vendor only)
// // =============================================================================
//
...
//                                 ),
//                               ],
//                               // Accept / Decline — vendor only, pending only
...
//         if (orderId != null) {
//           // Store orderId in SharedPreferences (matching the pattern from food_CartScreen)
//           final prefs = await SharedPreferences.getInstance();
...
//                 final orderId = result['orderId'] ?? result['id'];
//
//                 // Add cash billing entries for cash payments
...
//       await _printKOT(bookingId, itemsToUpdate);
//       await _fetchCartFromBackend();
//       _showSnack('✅ $updatedCount items delivered & printed!');
//     } catch (err) {
//       _showSnack('Failed: $err');
//     } finally {
//       setState(() => _isSaving = false);
...
//       await _printKOT(bookingId, itemsToUpdate);
//       await _fetchCartFromBackend();
//       _showSnack('✅ $updatedCount items KOT printed!');
//     } catch (err) {
//       _showSnack('Failed: $err');
//     } finally {
//       setState(() => _isKotPrinting = false);
...
//
//   Future<void> _printKOT(int bookingId, List<CartItem> items) async {
//     _showSnack('🖨️ KOT printed for ${items.length} items');
//   }
//
//   Future<int> _createBooking() async {
//     final id = await CartService.createBooking(
...
//
// // ─────────────────────────────────────────────────────────────────────────────
// // PENDING TABLE REQUESTS BOTTOM SHEET
// // ─────────────────────────────────────────────────────────────────────────────
//
// class _TableRequestsSheet extends StatelessWidget {
//   final List<TableRequestEntry> requests;
